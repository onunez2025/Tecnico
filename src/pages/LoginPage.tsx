...
import { User, Lock, Eye, EyeOff, Moon, Sun, LayoutDashboard } from 'lucide-react';
...
export default function LoginPage() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Suces, register session in Context
            login(data.user, data.token, rememberMe);

            // [FIX FE-H2] Navegar a '/' para que HomeRedirect maneje el ruteo por rol
            if (data.user.requires_password_change) {
                // [FIX UX-M10] Ruta /force-change-password no existe; redirigir a / como fallback seguro
                navigate('/');
            } else {
                navigate('/');
            }
    ...
    return (
        <div className="min-h-screen flex flex-col md:flex-row bg-background text-foreground transition-colors duration-300">
            {/* Left Side - Brand / Visual (Exactly matched to reference projects) */}
            <div className="hidden md:flex flex-col justify-between w-1/2 bg-slate-900 text-white p-12 relative overflow-hidden">
                {/* Abstract Background Pattern (Matches sister projects exactly) */}
                ...
                <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <span className="text-2xl font-bold tracking-tight">Gestión Técnica</span>
                    </div>
            ...
            {/* Right Side - Login Form (Exactly matched to reference projects) */}
                ...
                <div className="w-full max-w-md space-y-8">
                    ...
                    <form onSubmit={handleLogin} className="space-y-6 mt-8">
                        ...
                        <div className="flex items-center justify-between text-sm">
                            ...
                            <button
                                type="button"
                                onClick={() => setError('Contacta al Administrador de Redes para recuperar tu clave.')}
